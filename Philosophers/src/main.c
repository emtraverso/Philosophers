...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/14 14:05:06 by etravers          #+#    #+#             */
/*   Updated: 2023/04/14 14:05:09 by etravers         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_simposio(void *ph)
{
	...
	while (!check_morte(philo->data) || !philo->data->vaffanculo)
	{
		ft_eat(philo);
		if (philo->data->vaffanculo || check_morte(philo->data))
			break ;
		ft_sleep(philo);
		if (philo->data->vaffanculo || check_morte(philo->data))
			break ;
